// Load products and initialize database on page load
      ...
      function loadGallery() {
        ...
        const allPossibleImages = [...new Set(allProjectProducts.map(p => {
        ...
        const availableImages = allPossibleImages.filter(img => !usedImages.includes(img));
        const container = document.getElementById('galleryContainer');
        
        if (availableImages.length === 0) {
          container.innerHTML = '<p class="col-span-full text-gray-500 text-center py-8">All gallery images are in use! Add custom products above.</p>';
          return;
        }
      ...
      function openQuickAddModal(imageName) {
        ...
        document.getElementById('quickProductName').value = imageName.replace(/\.[^/.]+$/, '').replace(/\d+$/, '').trim();
      ...
      function loadProducts() {
        ...
        if (products.length === 0) {
          productsList.innerHTML = '<p class="col-span-full text-gray-500 text-center py-8">No products found</p>';
          return;
        }
        ...
        productsList.innerHTML = displayProducts.map(product => {
...
function initializeProductDatabase() {
  ...
  if (!hasSeeded && typeof allProjectProducts !== 'undefined') {
    const seededProducts = allProjectProducts.map((p, index) => ({
      ...
      price: p.price,
      category: p.category,
      image: p.image.startsWith('/') ? p.image.substring(1) : p.image.startsWith('./') ? p.image.substring(2) : p.image,
    ...
    console.log(`Initialized database with ${seededProducts.length} base products and ${existingProducts.length} user products.`);
  }
}
...
function saveProduct(product) {
  try {
    ...
    if (error.name === 'QuotaExceededError' || error.name === 'NS_ERROR_DOM_QUOTA_REACHED') {
      alert('Error: Storage limit reached! This usually happens when uploading large images. Try using a smaller image or picking from the gallery.');
    } else {
...
function updateWishlistButtons() {
  const wishlist = (JSON.parse(localStorage.getItem('wishlist')) || []).map(id => String(id));
